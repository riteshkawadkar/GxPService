...
namespace GxPService.Helpers
{
    internal class ApplicationLauncher
    {
        ...
        public static bool CreateProcessInConsoleSession(String CommandLine, bool bElevate)
        {
            ...
            log.Info($"CreateProcessInConsoleSession Started with cmd {CommandLine}");
            dwSessionId = WTSGetActiveConsoleSessionId();
            log.Info($"Active Console Session ID: {dwSessionId}");
            ...
            do
            {
                log.Info($"Checking process: {procEntry.szExeFile}, PID: {procEntry.th32ProcessID}");

                if (procEntry.szExeFile.IndexOf("winlogon.exe", StringComparison.OrdinalIgnoreCase) == 0)
                {
                    uint winlogonSessId = 0;
                    if (ProcessIdToSessionId(procEntry.th32ProcessID, ref winlogonSessId) && winlogonSessId == dwSessionId)
                    {
                        winlogonPid = procEntry.th32ProcessID;
                        log.Info($"Found winlogon process. PID: {winlogonPid}");
                        break;
                    }
                }
            }
